...
void extract_plots()
{
	...
	TH1D *velocity_hist =new TH1D("","Velocity distribution; velocity (km/s); Events",100,0,800);
        ...
        TH2D *xypos_hist=new TH2D("","Primary vertex distribution (X-Y); x (cm); y (cm);Events",100,-2,2,100,-2,2);
	...
	for(int i=0;i<30;i++)
{
   ...
   edep_arr.push_back(Edep_sum);
	...
	palette->SetX1NDC(0.91);
        ...
        c1->Clear();
	c1->SetLogz();
	TESxypos_hist->GetZaxis()->SetRangeUser(5e-2,1);
  ...
  TH1D *temperature_hist=new TH1D("","Expected temperature increase per event; T (#mu K)",100,0,400);
