...
void RunAction::BeginOfRunAction(const G4Run*)
{
  ...
  //--------------------For PMT------------------------//
  // analysisManager->OpenFile("Scintillation_data_pere_event.root");

    // Create Ntuple (Name, Title)
    analysisManager->CreateNtuple("Hits", "PMT Data Per Event");
    analysisManager->CreateNtupleIColumn("EventID");      // Column 0 (Integer)
    analysisManager->CreateNtupleIColumn("HitCount");     // Column 1 (Integer)
    analysisManager->CreateNtupleDColumn("TotalEnergyperEvent");  // Column 2
    // analysisManager->CreateNtupleDColumn("IndividualEnergies");   
     // analysisManager->CreateNtupleDColumn("InitialEnergy"); // Column 3 (NEW)
    analysisManager->FinishNtuple(0);

    // --- Ntuple 1: Photon Data ---
    analysisManager->CreateNtuple("Photons", "All Individual Hits");
    analysisManager->CreateNtupleDColumn("PhotonEn");    // Col 0
    analysisManager->FinishNtuple(1); // Explicitly ID this as 1
// }
    // Create Ntuple (We assign this Ntuple an ID of 0)
