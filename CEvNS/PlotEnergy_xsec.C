...
void PlotEnergy_xsec() {
    ...
    mg->SetTitle("Recoil Energy vs Differential Cross Section (Analytical);Energy (keV);Differential Cross Section (fm^{2})");

    // Define file names and visual preferences
    const char* files[] = {"dsig_vs_T_BGO_Bi.txt", "dsig_vs_T_BGO_Ge.txt", "dsig_vs_T_BGO_O.txt"};
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        mg->Add(gr, "L"); // "LP" means draw Line and Points
    ...
    mg->Draw("AL"); // 'A' is required for the first draw to create axes
    leg->Draw();
